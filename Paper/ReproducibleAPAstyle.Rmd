...
# Results
All analyses were conducted with `r my_citation`.  
...
```{r fig1, fig.cap = "Bar chart of condition means. Bars represent means, and error bars represent 95% confidence intervals.", echo=FALSE, dev.args = list(bg = 'white')}
expdata %>%
  mutate(bio_sex = as.factor(bio_sex),
         gender_expression = as.factor(gender_expression),
         gender_expression = factor(gender_expression, 
                                    labels=c("Masculine", "Feminine"))) %>%
ggplot(aes(x = bio_sex, y = hirability, fill = gender_expression)) +
