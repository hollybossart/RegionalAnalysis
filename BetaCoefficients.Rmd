...
language <-
  languageraw %>%
  mutate(Oblast = derivedVariable("Crimea" = rowname == 1 | rowname == 27,
                                "Kiev City" = rowname == 2,
                                "Kiev" = rowname == 3,
                                "Vinnytsya" = rowname == 4,
                                "Volyn" = rowname == 5,
...
# Create one large data set by joining with oblasts

combined.data =
  cidi %>%
  inner_join(corruption, by = "Oblast")  

##TODO: join all other data sets with the cidi data to make a large master data set by oblast
...
```{r}
# find bins, visualize
quantile(gdp$PerCapitaGDP)
ggplot(gdp, aes(x = PerCapitaGDP)) +
  geom_histogram() +
